...
#		return
#	_update_navigation_path(root.position, get_global_mouse_position())
...
func move_along_path(distance):
	var last_point = root.position 

	while path.size():
		var distance_between_points = last_point.distance_to(path[0])
		
		if distance <= distance_between_points:
			root.position =  last_point.lerp(path[0], distance / distance_between_points)
	...
	if path.size() == 0 and last_point != root.position:
		root.position = last_point
		set_idle()
...
func initiate_random_path():
	...
	var random_target = root.position + random_offset
	
	_update_navigation_path(root.position, random_target)
	
func enter_state(delta):
	# Logic when entering the wander state
	wander(delta)
